...
* Microservices "core" :
	...
	** *applications cloud-native* un bon schéma et une def courte et claire "L'approche Cloud-native est la façon dont les applications sont créées et déployées, non pas où elles sont exécutées." : https://www.supinfo.com/articles/single/5015-qu-est-ce-qu-une-application-cloud-native +
	J'aime également cette définition succincte des applications cloud native : *RESTful APIs that communicate over HTTP with JSON serialization* (https://www.slideshare.net/fortyfivan/microservices-webinar)
...
* Cast Codeurs 207, vérifier, mais apparemment l'implémentation Thorntail de MicroProfile (en v4 actuellement) vis des moments difficiles (à 01:22:00)
...
Définition d'Alistair COCKBURN + reminder p13
...
On peut parler pour cet environnement de "capability model" (voir https://hub.packtpub.com/capability-model-microservices/)
...
C'est bien ce type de comm qui est préconisé la plupart du temps pour les microservices -> CHECKER QU'ON L'INTEGRE BIEN DANS LA DEMO

-> pour ce point, voir https://dzone.com/articles/how-are-your-microservices-talking, qui repropose également une def des microservices
...
Cf l1 p34 : définition de Spring Boot et Cloud
...
1. rapide définition de ce qu'est un microservice et un peu d'histoire
	** def d'Alistair et un rien d'histoire
...
2. le "core" des microservices
	** principaux concepts : REST et communication en JSON
...
-> on ne va PAS pouvoir TOUT tester, mais on va essayer de montrer comment le "core" est réalisée pour chaque stack, quelle est un peu "sa philosophie", et COMMENT ELLE S'ADAPTE / PERMET LA MISE EN PLACE DU CAPACITY MODEL
	...
	** Demo
		**** QUE VA FAIRE L'APPLICATION DE DEMO !!!! +
		...
		Principe : 2 équipes de super héros vont faire la course pour récupérer les gemmes du gantelet +
...
-> Les applications cloud (native ?) sont un 1er use case, le 2nd étant la migration d'un monolithe vers une application microservices +
Sur ce point, voir p6 du l1
