...
-> On peut en final créer un Uberjar avec MicroProfile "complet" (je rappelle que toutes ses spécifications sont dédiées au Microservices), OU seulement avec certaines de ses briques ("fractions" comme les nomme Thorntail), comme MicroProfile OpenTracing
...
1) just enough application server
	...
	** réutiliser le schéma l2 p42 en faisant apparaître la notion d'Uberjar ? -> SCHEMA 2
