...
Nous commençons par le codage d'un premier microservice, qui, techniquement, n'est qu'un service "classique".
L'une de ses principales caractéristiques est de disposer d'une *persistance dédiée*.
...
Les microservices peuvent communiquer / être appelés de 2 façons différentes, *synchrone* ou *asynchrone*.
